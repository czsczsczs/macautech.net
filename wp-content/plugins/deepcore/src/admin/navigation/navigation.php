...
class wn_description_walker extends Walker_Nav_Menu{
    ...
    function start_el(&$output, $item, $depth=0, $args=array(),$current_object_id=0){
        ...
        foreach ( $atts as $attr => $value ) {
            if ( ! empty( $value ) ) {
                $result = substr($current_url,strpos($current_url,'?'));
                $value_s = substr($value,strpos($value,'?'));
                $current_color = (!empty($current_url)&& $result == $value_s ) ? 'style="color:#6bb24d !important;"' : "";
        ...
        if('page'  == $item->object){
            ...
            $item_output .= '<a '. $attributes . $current_color . ' data-description="' .$item->description .'">';
            if(!empty($item->icon))
                $item_output .= '<i class="'.$item->icon.' wn-menu-icon"></i>';
