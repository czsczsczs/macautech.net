...
function render_user_permissions_settings_fields_premium() {
	...
	// jasper20210830 1009 获取所有用户角色
	$roles      = $wp_roles->get_names();
	// jasper20210830 0926 获取数据库option表的 ig_es_user_roles 的值
	$user_roles = get_option('ig_es_user_roles',array());

	$url = ES_Common::get_utm_tracking_url( array( 'utm_medium' => 'user_roles' ) );

	// $sub_menus = array('dashboard','workflows','audience','reports','forms','campaigns','sequences','settings','ig_redirect',);
			...
			foreach ( $roles as $key => $value ) {
				...
				<tr class="border-b border-gray-200">
					...
					<td class="whitespace-no-wrap text-center">
						<!-- jasper20210826 0940 -->
						<input type="checkbox" name="ig_es_user_roles[audience][<?php echo esc_html( $key ); ?>]" <?php ! empty( $user_roles['audience'][ $key ] ) ? checked( 'yes', $user_roles['audience'][ $key ] ) : ''; ?> value="yes" class=" form-checkbox text-indigo-600">
					</td>
					<td class="whitespace-no-wrap text-center">
						<!-- jasper20210826 0941 -->
						<input type="checkbox" name="ig_es_user_roles[forms][<?php echo esc_html( $key ); ?>]" <?php ! empty( $user_roles['forms'][ $key ] ) ? checked( 'yes', $user_roles['forms'][ $key ] ) : ''; ?> value="yes" class=" form-checkbox text-indigo-600">
					</td>
					<td class="whitespace-no-wrap text-center">
						<!-- jasper20210826 0942 -->
						<input type="checkbox" name="ig_es_user_roles[campaigns][<?php echo esc_html( $key ); ?>]" <?php ! empty( $user_roles['campaigns'][ $key ] ) ? checked( 'yes', $user_roles['campaigns'][ $key ] ) : ''; ?> value="yes" class=" form-checkbox text-indigo-600">
					</td>
					<td class="whitespace-no-wrap text-center">
						<!-- jasper20210826 0943 -->
						<input type="checkbox" name="ig_es_user_roles[reports][<?php echo esc_html( $key ); ?>]" <?php ! empty( $user_roles['reports'][ $key ] ) ? checked( 'yes', $user_roles['reports'][ $key ] ) : ''; ?> value="yes" class=" form-checkbox text-indigo-600">
					</td>
					<td class="whitespace-no-wrap text-center">
						<!-- jasper20210826 0944 -->
						<input type="checkbox" name="ig_es_user_roles[sequences][<?php echo esc_html( $key ); ?>]" <?php ! empty( $user_roles['sequences'][ $key ] ) ? checked( 'yes', $user_roles['sequences'][ $key ] ) : ''; ?> value="yes" class=" form-checkbox text-indigo-600">
					</td>
					<td class="whitespace-no-wrap text-center">
						<!-- jasper20210826 0945 -->
						<input type="checkbox" name="ig_es_user_roles[workflows][<?php echo esc_html( $key ); ?>]" <?php ! empty( $user_roles['workflows'][ $key ] ) ? checked( 'yes', $user_roles['workflows'][ $key ] ) : ''; ?> value="yes" class=" form-checkbox text-indigo-600">
					</td>
				</tr>
				<?php
			}
