...
class ES_Templates_Table {
	...
	public function es_template_type_meta_box( $post ) {
		...
		{{POSTAUTHOR}}, {{POSTLINK}}, {{POSTLINK-WITHTITLE}}, {{POSTLINK-ONLY}}, {{POSTFULL}}, {{POST-STARTDATE}}, {{POST-ENDDATE}}, {{POSTORGANIZER}}, {{POSTLOCATION}}</p>
		<!-- End-IG-Code -->
		<p id="newsletter">
	...
	public function es_template_preview_callback() {
		...
		if ( $template ) {
			...
			//根据当前用户的邮箱匹配插件里面相同邮箱的订阅者，并获取对应id
			...
			if ( 'post_notification' === $es_template_type ) {
				...
				$recent_posts = wp_get_recent_posts( $args );//获取最近的帖子
