...
class ES_Shortcode {
	...
	public static function render_form( $data ) {
		...
		if ( ! empty( $show_name ) && 'no' !== $show_name ) {
			...
			$name_html .= '<div class="es-field-wrap"><!-- jasper20211129 1624 --><label><input type="text" name="esfpx_name" class="ig_es_form_field_name es_ipt_jp"  placeholder="' . $name_place_holder . '" value="' . $submitted_name . '" ' ;
		...
		if ( ! empty( $list_ids ) && $show_list ) {
			...
			$list_html         = self::prepare_lists_checkboxes(0, $lists_id_name_map, $list_ids, 1, $selected_list_ids, 0, 'esfpx_lists[]', $lists_id_hash_map );
		...
		$email_html .= '<!-- jasper20211129 1626 --><input class="es_required_field es_txt_email ig_es_form_field_email es_ipt_jp" type="email" name="esfpx_email" value="' . $submitted_email . '" placeholder="' . $email_place_holder . '" required="required"/></label></div>';
		?>
		<style type="text/css">
			.emaillist{
			    padding-top: 40px;
		...
		<div class="emaillist" id="es_form_<?php echo esc_attr( self::$form_identifier ); ?>">
			...
			if ( 'success' !== $message_class) {
				...
				?>
			<form action="<?php echo esc_url( $form_action_url ); ?>#es_form_<?php echo esc_attr( self::$form_identifier ); ?>" method="post" class="es_subscription_form es_shortcode_form es_form_jp" id="es_subscription_form_<?php echo esc_attr( $unique_id ); ?>" data-source="ig-es" target="_self">
				<!-- jasper20211129 1627 -->
				<?php if ( '' != $desc ) { ?>
					<div class="es_caption"><?php echo esc_html( $desc ); ?></div>
				<?php } ?>

				<link href=<?php echo plugin_dir_url( __DIR__ )."css/fSelect.css"?> rel="stylesheet" type="text/css">
				<script type="text/javascript" src=<?php echo plugin_dir_url( __DIR__ )."js/jquery-1.11.3.min.js"?>></script>
				<script type="text/javascript" src=<?php echo plugin_dir_url( __DIR__ )."js/show_ads.js"?>></script>
				<?php
				...
				?>
				<script type="text/javascript" src=<?php echo plugin_dir_url( __DIR__ )."js/fSelect.js"?>></script>
				<script type="text/javascript">jQuery(function(){ jQuery('.demo').fSelect(); });</script>
				...
				?>
				<!-- jasper20211129 1633 -->
				<input type="submit" class="es_subscription_form_submit es_submit_button es_textbox_button es_btn_jp" id="es_subscription_form_submit_<?php echo esc_attr( $unique_id ); ?> btn-loading-example" value="<?php echo esc_attr( $button_label ); ?>" style="padding: 16.5px 20px;border-radius: 3px;background: #84C7E8;" onclick="this.disabled='true';this.form.submit();"/>
			...
			?>
			<span class="es_subscription_message <?php echo esc_attr( $message_class ); ?> es_form_span_jp" id="es_subscription_message_<?php echo esc_attr( $unique_id ); ?>">
	...
	/**
	 * 生成前端用户选择关注分类列表
	 * @param  [type]  $lists             分类名称
	 * @param  array   $list_ids          分类id
	 * @param  integer $columns           [description]
	 * @param  array   $selected_lists    [description]
	 * @param  integer $contact_id        [description]
	 * @param  string  $name              数据名称
	 * @param  array   $lists_id_hash_map 分类hash码
	 * @return [type]                     [description]
	 */
	public static function prepare_lists_checkboxes($jptype = 0, $lists, $list_ids = array(), $columns = 3, $selected_lists = array(), $contact_id = 0, $name = 'lists[]', $lists_id_hash_map = array() ) {

		// 判断contact_id是否为空，不为空显示标题
		$lists_html = '';
		...
		if ( ! empty( $jptype ) ) {
			...
			$lists_html .= '<input type="hidden" name="esfpx_list" class="esfpx_list" value=""/>';
			$lists_html .= '<div id="main_demo">';
			$lists_html .= '<div class="demo_ctr">';
			$lists_html .= '<select class="demo" multiple="multiple">';
			$lists_html .= '<optgroup label="活動分類">';
			$i = 0;
			//巡环输出<optgroup label="請選擇想要訂閱的分類">分类表单列表
			foreach ( $lists as $list_id => $list_name ) {
				$status_span = '';
				if ( in_array( $list_id, $list_ids ) ) {

					// 检查列表哈希是否已传递给给定的列表id，如果是，则使用列表哈希，否则使用列表id
					if ( ! empty( $lists_id_hash_map[ $list_id ] ) ) {
						$list_value = $lists_id_hash_map[ $list_id ];
					} else {
						$list_value = $list_id;
					...
					$lists_html .= '<option value="' . $list_value . '" >' . $list_name . '</option>';
					$i ++;
				}
			}
			$lists_html .= '</optgroup>';
